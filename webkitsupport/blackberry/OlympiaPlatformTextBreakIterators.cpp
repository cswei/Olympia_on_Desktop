/* copying from TextBreakIteratorICU.cpp
 *
...
#define currentTextBreakLocale "en_US"
...
TextBreakIterator* setUpIterator(bool& createdIterator, TextBreakIterator* iterator,
    UBreakIteratorType type, const unsigned short* string, int length)
{
    if (!string)
        return 0;
    ...
    if (!createdIterator) {
        ...
        // iterator->m_iterator = ubrk_open(type, WebCore::currentTextBreakLocaleID(), 0, 0, &openStatus);
        iterator->m_iterator = ubrk_open(type, currentTextBreakLocale, 0, 0, &openStatus);
    ...
    ubrk_setText(iterator->m_iterator, string, length, &setTextStatus);
...
TextBreakIterator* characterBreakIterator(const unsigned short* string, int length)
{
...
TextBreakIterator* wordBreakIterator(const unsigned short* string, int length)
{
...
TextBreakIterator* lineBreakIterator(const unsigned short* string, int length)
{
...
TextBreakIterator* sentenceBreakIterator(const unsigned short* string, int length)
{
...
{
    return ubrk_first(iterator->m_iterator);
}

int textBreakLast(TextBreakIterator* iterator)
{
    return ubrk_last(iterator->m_iterator);
}

int textBreakNext(TextBreakIterator* iterator)
{
    return ubrk_next(iterator->m_iterator);
}

int textBreakPrevious(TextBreakIterator* iterator)
{
    return ubrk_previous(iterator->m_iterator);
}

int textBreakPreceding(TextBreakIterator* iterator, int pos)
{
    return ubrk_preceding(iterator->m_iterator, pos);
}

int textBreakFollowing(TextBreakIterator* iterator, int pos)
{
    return ubrk_following(iterator->m_iterator, pos);
}

int textBreakCurrent(TextBreakIterator* iterator)
{
    return ubrk_current(iterator->m_iterator);
}

bool isTextBreak(TextBreakIterator* iterator, int position)
{
    return ubrk_isBoundary(iterator->m_iterator, position);
}

TextBreakIterator* setUpIteratorWithRules(bool& createdIterator, TextBreakIterator* iterator,
    const char* breakRules, const unsigned short* string, int length)
{
    if (!string)
        return 0;
    ...
    if (!createdIterator) {
        ...
        int len = strlen(breakRules);
        unsigned short* data = static_cast<unsigned short*>(malloc(len * sizeof(unsigned short)));
        for (unsigned i = 0; i != len; ++i) {
            unsigned char c = breakRules[i];
            data[i] = c;
        }
        iterator->m_iterator = ubrk_openRules(data, len, 0, 0, &parseStatus, &openStatus);
        createdIterator = true;
        free(data);
        ASSERT_WITH_MESSAGE(U_SUCCESS(openStatus), "ICU could not open a break iterator: %s (%d)", u_errorName(openStatus), openStatus);
    }
...
TextBreakIterator* cursorMovementIterator(const unsigned short* string, int length)
{
