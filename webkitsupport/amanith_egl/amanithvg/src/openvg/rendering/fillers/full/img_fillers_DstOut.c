...
/*!
	\file img_fillers_DstOut.c
	\brief VG_BLEND_DST_OUT_MZT image fillers, implementation ( \b generated \b file ).
...
/*!
	\brief Image filler: blend mode VG_BLEND_DST_OUT_MZT, image mode VG_DRAW_IMAGE_NORMAL, affine transformation ( \b generated \b function ).
...
/*!
	\brief Image filler: blend mode VG_BLEND_DST_OUT_MZT, image mode VG_DRAW_IMAGE_NORMAL, perspective transformation ( \b generated \b function ).
	\param _surface pointer to a AMDrawingSurface structure, defining the destination drawing surface.
	\param _paintGen pointer to a AMPaintGen structure, containing paint information and derived values.
	\param y y coordinate of the scanline to fill.
	\param x0 x coordinate of the left-most pixel to fill.
...
loop_NormalProjectiveDstOut:
	...
	if (invCov == 0) {
		...
		scrPixels[j++] = tmp.value;
		if (--i != 0)
			goto loop_NormalProjectiveDstOut;
		else {
			covLine[j] = 0;
			return;
		}
...
/*!
	\brief Image filler: blend mode VG_BLEND_DST_OUT_MZT, image mode VG_DRAW_IMAGE_MULTIPLY, affine transformation ( \b generated \b function ).
	\param _surface pointer to a AMDrawingSurface structure, defining the destination drawing surface.
	\param _paintGen pointer to a AMPaintGen structure, containing paint information and derived values.
	\param y y coordinate of the scanline to fill.
	\param x0 x coordinate of the left-most pixel to fill.
...
loop_MultiplyAffineDstOut:
	...
	if (invCov == 0) {
		...
		scrPixels[j++] = tmp.value;
		if (--i != 0)
			goto loop_MultiplyAffineDstOut;
		else {
			covLine[j] = 0;
			return;
		}
...
/*!
	\brief Image filler: blend mode VG_BLEND_DST_OUT_MZT, image mode VG_DRAW_IMAGE_STENCIL, affine transformation ( \b generated \b function ).
	\param _surface pointer to a AMDrawingSurface structure, defining the destination drawing surface.
	\param _paintGen pointer to a AMPaintGen structure, containing paint information and derived values.
	\param y y coordinate of the scanline to fill.
	\param x0 x coordinate of the left-most pixel to fill.
...
loop_StencilAffineDstOut:
	...
	DcaDa.value = scrPixels[j];
		...
		tmp.value = amPxlMul(invSa.value, DcaDa.value);
