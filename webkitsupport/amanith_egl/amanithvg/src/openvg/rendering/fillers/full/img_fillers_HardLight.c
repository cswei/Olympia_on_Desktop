...
/*!
	\file img_fillers_HardLight.c
	\brief VG_BLEND_HARD_LIGHT_MZT image fillers, implementation ( \b generated \b file ).
...
/*!
	\brief Image filler: blend mode VG_BLEND_HARD_LIGHT_MZT, image mode VG_DRAW_IMAGE_NORMAL, affine transformation ( \b generated \b function ).
...
loop_NormalAffineHardLight:
	...
	if (invCov == 0) {
		...
		if (--i != 0)
			goto loop_NormalAffineHardLight;
		else {
			covLine[j] = 0;
			return;
		}
...
/*!
	\brief Image filler: blend mode VG_BLEND_HARD_LIGHT_MZT, image mode VG_DRAW_IMAGE_NORMAL, perspective transformation ( \b generated \b function ).
	\param _surface pointer to a AMDrawingSurface structure, defining the destination drawing surface.
	\param _paintGen pointer to a AMPaintGen structure, containing paint information and derived values.
	\param y y coordinate of the scanline to fill.
	\param x0 x coordinate of the left-most pixel to fill.
...
loop_NormalProjectiveHardLight:
	// update coverage and potential mask (according to alpha mask)
	cov += covLine[j];
	if (covLine[j] != 0)
		covLine[j] = 0;
	...
	if (invCov == 0) {
		...
		scrPixels[j++] = tmp.value;
		if (--i != 0)
			goto loop_NormalProjectiveHardLight;
		else {
			covLine[j] = 0;
			return;
		}
...
/*!
	\brief Image filler: blend mode VG_BLEND_HARD_LIGHT_MZT, image mode VG_DRAW_IMAGE_MULTIPLY, affine transformation ( \b generated \b function ).
	\param _surface pointer to a AMDrawingSurface structure, defining the destination drawing surface.
	\param _paintGen pointer to a AMPaintGen structure, containing paint information and derived values.
	\param y y coordinate of the scanline to fill.
	\param x0 x coordinate of the left-most pixel to fill.
...
loop_MultiplyAffineHardLight:
	...
	if (invCov == 0) {
		...
		scrPixels[j++] = tmp.value;
		if (--i != 0)
			goto loop_MultiplyAffineHardLight;
		else {
			covLine[j] = 0;
			return;
		}
...
/*!
	\brief Image filler: blend mode VG_BLEND_HARD_LIGHT_MZT, image mode VG_DRAW_IMAGE_STENCIL, affine transformation ( \b generated \b function ).
	\param _surface pointer to a AMDrawingSurface structure, defining the destination drawing surface.
	\param _paintGen pointer to a AMPaintGen structure, containing paint information and derived values.
	\param y y coordinate of the scanline to fill.
	\param x0 x coordinate of the left-most pixel to fill.
...
loop_StencilAffineHardLight:
	...
	DcaDa.value = scrPixels[j];
		...
		t2.value = t0.value + t1.value;                  // Sca.(1 - Da) + Dca.(1 - Sa)
		t3.value = amPxlMulNoA(ScaSa.value, DcaDa.value);  // ScaSa.DcaDa
		t4 = ScaSa;
		t5 = DcaDa;

		B = (AMint32)t4.c.b * 2 - Sa.c.b;
		if (B < 0) {
			B = (AMint32)t3.c.b * 2 + t2.c.b;
			if (B > 255)
				B = 255;
			...
			B = ((((AMint32)t4.c.b * t5.c.a + (AMint32)t5.c.b * Sa.c.b) * 2) >> 8) - (((AMint32)Sa.c.b * t5.c.a) >> 8) - (AMint32)t3.c.b * 2 + t2.c.b;
			B = AM_CLAMP(B, 0, 255);
		}

		G = (AMint32)t4.c.g * 2 - Sa.c.g;
		if (G < 0) {
			G = (AMint32)t3.c.g * 2 + t2.c.g;
			if (G > 255)
				G = 255;
			...
			G = ((((AMint32)t4.c.g * t5.c.a + (AMint32)t5.c.g * Sa.c.g) * 2) >> 8) - (((AMint32)Sa.c.g * t5.c.a) >> 8) - (AMint32)t3.c.g * 2 + t2.c.g;
			G = AM_CLAMP(G, 0, 255);
		}

		R = (AMint32)t4.c.r * 2 - Sa.c.r;
		if (R < 0) {
			R = (AMint32)t3.c.r * 2 + t2.c.r;
			if (R > 255)
				R = 255;
			...
			R = ((((AMint32)t4.c.r * t5.c.a + (AMint32)t5.c.r * Sa.c.r) * 2) >> 8) - (((AMint32)Sa.c.r * t5.c.a) >> 8) - (AMint32)t3.c.r * 2 + t2.c.r;
			R = AM_CLAMP(R, 0, 255);
		}
	...
	if (invCov == 0) {
		...
		scrPixels[j++] = tmp.value;
		if (--i != 0)
			goto loop_StencilAffineHardLight;
		else {
			covLine[j] = 0;
			return;
		}
