...
/*!
	\file img_fillers_SrcAtop.c
	\brief VG_BLEND_SRC_ATOP_MZT image fillers, implementation ( \b generated \b file ).
...
/*!
	\brief Image filler: blend mode VG_BLEND_SRC_ATOP_MZT, image mode VG_DRAW_IMAGE_NORMAL, affine transformation ( \b generated \b function ).
...
/*!
	\brief Image filler: blend mode VG_BLEND_SRC_ATOP_MZT, image mode VG_DRAW_IMAGE_NORMAL, perspective transformation ( \b generated \b function ).
	\param _surface pointer to a AMDrawingSurface structure, defining the destination drawing surface.
	\param _paintGen pointer to a AMPaintGen structure, containing paint information and derived values.
	\param y y coordinate of the scanline to fill.
	\param x0 x coordinate of the left-most pixel to fill.
...
loop_NormalProjectiveSrcAtop:
	...
	if (invCov == 0) {
		...
		scrPixels[j++] = tmp.value;
		if (--i != 0)
			goto loop_NormalProjectiveSrcAtop;
		else {
			covLine[j] = 0;
			return;
		}
...
/*!
	\brief Image filler: blend mode VG_BLEND_SRC_ATOP_MZT, image mode VG_DRAW_IMAGE_MULTIPLY, affine transformation ( \b generated \b function ).
	\param _surface pointer to a AMDrawingSurface structure, defining the destination drawing surface.
	\param _paintGen pointer to a AMPaintGen structure, containing paint information and derived values.
	\param y y coordinate of the scanline to fill.
	\param x0 x coordinate of the left-most pixel to fill.
...
loop_MultiplyAffineSrcAtop:
	...
	if (invCov == 0) {
		...
		scrPixels[j++] = tmp.value;
		if (--i != 0)
			goto loop_MultiplyAffineSrcAtop;
		else {
			covLine[j] = 0;
			return;
		}
...
/*!
	\brief Image filler: blend mode VG_BLEND_SRC_ATOP_MZT, image mode VG_DRAW_IMAGE_STENCIL, affine transformation ( \b generated \b function ).
	\param _surface pointer to a AMDrawingSurface structure, defining the destination drawing surface.
	\param _paintGen pointer to a AMPaintGen structure, containing paint information and derived values.
	\param y y coordinate of the scanline to fill.
	\param x0 x coordinate of the left-most pixel to fill.
...
loop_StencilAffineSrcAtop:
	...
	DcaDa.value = scrPixels[j];
		...
		t0.value = amPxlScl255(DcaDa.c.a, ScaSa.value);
		t1.value = amPxlMulNoA(invSa.value, DcaDa.value);
		tmp.value = t0.value + t1.value;
		tmp.c.a = DcaDa.c.a;
	...
	if (invCov == 0) {
		...
		scrPixels[j++] = tmp.value;
		if (--i != 0)
			goto loop_StencilAffineSrcAtop;
		else {
			covLine[j] = 0;
			return;
		}
