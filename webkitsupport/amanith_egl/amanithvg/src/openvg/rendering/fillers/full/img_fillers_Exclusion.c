...
/*!
	\file img_fillers_Exclusion.c
	\brief VG_BLEND_EXCLUSION_MZT image fillers, implementation ( \b generated \b file ).
...
/*!
	\brief Image filler: blend mode VG_BLEND_EXCLUSION_MZT, image mode VG_DRAW_IMAGE_NORMAL, affine transformation ( \b generated \b function ).
...
loop_NormalAffineExclusion:
	...
	if (invCov == 0) {
		...
		if (--i != 0)
			goto loop_NormalAffineExclusion;
		else {
			covLine[j] = 0;
			return;
		}
...
/*!
	\brief Image filler: blend mode VG_BLEND_EXCLUSION_MZT, image mode VG_DRAW_IMAGE_NORMAL, perspective transformation ( \b generated \b function ).
	\param _surface pointer to a AMDrawingSurface structure, defining the destination drawing surface.
	\param _paintGen pointer to a AMPaintGen structure, containing paint information and derived values.
	\param y y coordinate of the scanline to fill.
	\param x0 x coordinate of the left-most pixel to fill.
...
loop_NormalProjectiveExclusion:
	// update coverage and potential mask (according to alpha mask)
	cov += covLine[j];
	if (covLine[j] != 0)
		covLine[j] = 0;
	...
	if (invCov == 0) {
		...
		scrPixels[j++] = tmp.value;
		if (--i != 0)
			goto loop_NormalProjectiveExclusion;
		else {
			covLine[j] = 0;
			return;
		}
...
/*!
	\brief Image filler: blend mode VG_BLEND_EXCLUSION_MZT, image mode VG_DRAW_IMAGE_MULTIPLY, affine transformation ( \b generated \b function ).
	\param _surface pointer to a AMDrawingSurface structure, defining the destination drawing surface.
	\param _paintGen pointer to a AMPaintGen structure, containing paint information and derived values.
	\param y y coordinate of the scanline to fill.
	\param x0 x coordinate of the left-most pixel to fill.
...
loop_MultiplyAffineExclusion:
	...
	if (invCov == 0) {
		...
		scrPixels[j++] = tmp.value;
		if (--i != 0)
			goto loop_MultiplyAffineExclusion;
		else {
			covLine[j] = 0;
			return;
		}
...
/*!
	\brief Image filler: blend mode VG_BLEND_EXCLUSION_MZT, image mode VG_DRAW_IMAGE_STENCIL, affine transformation ( \b generated \b function ).
	\param _surface pointer to a AMDrawingSurface structure, defining the destination drawing surface.
	\param _paintGen pointer to a AMPaintGen structure, containing paint information and derived values.
	\param y y coordinate of the scanline to fill.
	\param x0 x coordinate of the left-most pixel to fill.
...
loop_StencilAffineExclusion:
	...
	if (invCov == 0) {
		...
		scrPixels[j++] = tmp.value;
		if (--i != 0)
			goto loop_StencilAffineExclusion;
		else {
			covLine[j] = 0;
			return;
		}
