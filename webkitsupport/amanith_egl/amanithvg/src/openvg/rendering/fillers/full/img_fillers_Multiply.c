...
/*!
	\file img_fillers_Multiply.c
	\brief VG_BLEND_MULTIPLY image fillers, implementation ( \b generated \b file ).
...
/*!
	\brief Image filler: blend mode VG_BLEND_MULTIPLY, image mode VG_DRAW_IMAGE_NORMAL, affine transformation ( \b generated \b function ).
...
loop_NormalAffineMultiply:
	...
	if (invCov == 0) {
		...
		if (--i != 0)
			goto loop_NormalAffineMultiply;
		else {
			covLine[j] = 0;
			return;
		}
...
/*!
	\brief Image filler: blend mode VG_BLEND_MULTIPLY, image mode VG_DRAW_IMAGE_NORMAL, perspective transformation ( \b generated \b function ).
	\param _surface pointer to a AMDrawingSurface structure, defining the destination drawing surface.
	\param _paintGen pointer to a AMPaintGen structure, containing paint information and derived values.
	\param y y coordinate of the scanline to fill.
	\param x0 x coordinate of the left-most pixel to fill.
...
loop_NormalProjectiveMultiply:
	...
	if (invCov == 0) {
		...
		scrPixels[j++] = tmp.value;
		if (--i != 0)
			goto loop_NormalProjectiveMultiply;
		else {
			covLine[j] = 0;
			return;
		}
...
/*!
	\brief Image filler: blend mode VG_BLEND_MULTIPLY, image mode VG_DRAW_IMAGE_MULTIPLY, affine transformation ( \b generated \b function ).
	\param _surface pointer to a AMDrawingSurface structure, defining the destination drawing surface.
	\param _paintGen pointer to a AMPaintGen structure, containing paint information and derived values.
	\param y y coordinate of the scanline to fill.
	\param x0 x coordinate of the left-most pixel to fill.
...
loop_MultiplyAffineMultiply:
	...
	if (invCov == 0) {
		...
		scrPixels[j++] = tmp.value;
		if (--i != 0)
			goto loop_MultiplyAffineMultiply;
		else {
			covLine[j] = 0;
			return;
		}
...
/*!
	\brief Image filler: blend mode VG_BLEND_MULTIPLY, image mode VG_DRAW_IMAGE_STENCIL, affine transformation ( \b generated \b function ).
	\param _surface pointer to a AMDrawingSurface structure, defining the destination drawing surface.
	\param _paintGen pointer to a AMPaintGen structure, containing paint information and derived values.
	\param y y coordinate of the scanline to fill.
	\param x0 x coordinate of the left-most pixel to fill.
...
loop_StencilAffineMultiply:
	...
	DcaDa.value = scrPixels[j];
		...
		t0.value = amPxlMulNoA(ScaSa.value, DcaDa.value);
		t1.value = amPxlScl255(invDa, ScaSa.value);
		t2.value = amPxlMul(invSa.value, DcaDa.value);
		t0.value = amPxlAddsNoA(t0.value, t1.value);
		tmp.value = amPxlAddsNoA(t0.value, t2.value);
		tmp.c.a = ScaSa.c.a + t2.c.a;
	...
	if (invCov == 0) {
		...
		scrPixels[j++] = tmp.value;
		if (--i != 0)
			goto loop_StencilAffineMultiply;
		else {
			covLine[j] = 0;
			return;
		}
