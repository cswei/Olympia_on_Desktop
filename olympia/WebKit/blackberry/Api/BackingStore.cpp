...
void BackingStore::createSurface()
{
    ...
    // Implementation of EGL on Linux platform may return
    // different EGLDisplays when call eglGetDisplay(EGL_DEFAULT_DISPLAY)
    // mulitple times  because Linux may support multiple display
    // (e.g. Linux X11). So we just call EGLDisplayOpenVG::current()->display()
    // to make sure all BackingStores are using the same display.
    // If there are more one different displays SurfacePool and
    // EGLDisplayOpenVG::current()->sharedPlatformSurface() don't
    // work because OpenVG can't share context between different
    // EGLDisplays.
    d->m_eglDisplay = EGLDisplayOpenVG::current()->display();
#else
    // Get EGL display.
