...
#include <memalloc.h>
#else
#if !defined(_MSC_VER) && !defined(OLYMPIA_LINUX) && !defined(OLYMPIA_MAC)
//#include <i_egl.h>
#endif
#endif
...
bool WebPage::bitmapZoom(int x, int y, double scale, bool shouldZoomAboutPoint)
{
    ...
    if (d->m_bitmapScale < minimumScale)
        ...
        // Because the animateZoomBounceBack is triggered by the timer and it takes time to complete,
        // So if a new zoom event received while the animateZoomBounceBack has not finished,
        // the "delete d->m_bitmapZoomBuffer" would make the ongoing animation job draws on the invalid buffer.
        // So we should stop the animation work first before we proceed.
        if (d->m_zoomBounceBackAnimation->isRunning())
            d->m_zoomBounceBackAnimation->stop();
...
#if PLATFORM(OLYMPIA)
void WebPage::setScreenSize(const Platform::IntSize& screenSize, const Platform::IntSize& defaultLayoutSize, const Platform::IntRect& viewportRect)
{
    int screenWidth = screenSize.width();
    int screenHeight = screenSize.height();
    if (screenWidth == d->m_width && screenHeight == d->m_height)
    ...
    Olympia::Platform::log(Olympia::Platform::LogLevelInfo, "WebPage::setScreenSize screenWidth=%d, screenHeight=%d viewport=(%d,%d %dx%d)",
        screenWidth, screenHeight, viewportRect.x(), viewportRect.y(), viewportRect.width(), viewportRect.height());
